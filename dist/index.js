...
import { getVersionList, install, installFabric, installForge, installLibraries, installAssets, scanLocalJava, getForgeVersionList, } from "@xmcl/installer";
...
function getInstalledVersions() {
    ...
    if (!existsSync(dir))
        return [];
    return readdirSync(dir).filter((v) => existsSync(join(dir, v, `${v}.json`)));
}
function isVersionIsolated(versionId) {
...
function gameDir(versionId, isolated) {
    return isolated ? join(MC_ROOT, "versions", versionId) : MC_ROOT;
}
const runningProcesses = new Map();
const server = new Server({ name: "minecraft-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
...
async function handleGetVersionList(args) {
    const list = await getVersionList();
    const filtered = args?.type ? list.versions.filter((v) => v.type === args.type) : list.versions;
    const lines = [
        ...
        "",
        ...filtered.slice(0, 100).map((v) => `${v.id} (${v.type}) - ${v.releaseTime}`),
    ];
    return { content: [{ type: "text", text: lines.join("\n") }] };
}
async function handleInstallMinecraft(args) {
...
async function handleInstallFabric(args) {
...
async function handleInstallForge(args) {
    const mcVersion = args.minecraftVersion;
    const isolated = !!args.isolated;
    const list = await getVersionList();
    const versionMeta = list.versions.find((v) => v.id === mcVersion);
    ...
    if (!forgeVersion) {
        const forgeVersions = await getForgeVersionList({ minecraft: mcVersion });
        if (!forgeVersions || forgeVersions.length === 0) {
            throw new Error(`No Forge version available for Minecraft ${mcVersion}`);
        }
        const target = forgeVersions.find((v) => v.type === "Recommended") || forgeVersions[0];
        forgeVersion = target.version;
    }
    const versionId = await installForge({ mcversion: mcVersion, version: forgeVersion }, MC_ROOT, { side: "client" });
    if (isolated) {
        mkdirSync(join(MC_ROOT, "versions", versionId, "mods"), { recursive: true });
    }
    return {
...
async function handleLaunchMinecraft(args) {
    const versions = getInstalledVersions();
    if (versions.length === 0) {
        throw new Error("No versions installed. Use install_minecraft first.");
    }
    const versionId = args.version || (versions.includes(args.version) ? args.version : versions[0]);
    ...
    if (!javaPath) {
        const javas = await scanLocalJava([]);
        const suitable = javas.find((j) => j.majorVersion >= 17) || javas[0];
        if (!suitable) {
...
async function handleStopMinecraft(args) {
    const versionId = args.version;
...
async function handleReadLog(args) {
    const versionId = args.version;
...
async function handleCopyMod(args) {
    const versionId = args.version;
